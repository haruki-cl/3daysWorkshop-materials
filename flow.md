...
- **Claude Code CLI** — 本ワークショップの主役。起動時にサインインが要るので、**インストール＋ログインまで**済ませておく
...
自分宛にメールが届く／連投対策（クールダウン → **429**）が効く（本編 [9] の KV レート制限と同じ）。テストでは外部 fetch をモックし、実送信を起こさない。
...
未ログインで `/mypage` に入れない／`GET /api/me` が**認証必須**（トークン無し・偽造で弾かれる）。
...
レート制限が **429 を返すことをテストで証明**できる（余裕があれば運用スキルが一通り動く）。
...
- **意図しないリポジトリまで勝手に解析される**ことがある。原因は2つの組み合わせ:
  ...
  - → App が見える全リポジトリに push が走るたび、SonarCloud が**自分で登録していないプロジェクトまで自動生成して解析**する（チェック自体は通るので実害は無いが、紛らわしい）
  - **対処（確実なのは上）**:
