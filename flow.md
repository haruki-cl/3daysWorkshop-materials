...
| 0:35–1:00 | [1] Claude Design（LP 中心に3案 → 1つ選ぶ → 他ページを追加生成） |
| 1:00–1:55 | [2] フロント作成（デザイン zip を渡して全画面・モックで動かす） |
| 1:55–2:00 | クロージング: 次回予告・dev に push 確認 |
...
| 0:10–1:30 | オプション課題（興味のあるものを1つ。Day 2 が未了の人はその続きでも可） |
...
> 0.6 では入門ではなく **Claude Code を安全に使うためのセキュリティ対策**だけを扱う。
...
| 5 | （任意）オプション機能を追加して発表する | Day 3 | 最終発表会 |

> **必達は Day 2 のデプロイまで（ゴール 1〜4）**。各日2時間と短いので、全員でここをやり切ることに集中する。
> **ゴール 5（Day 3 のオプション）は必須ではない**——やらない／1つだけ、も可。興味に応じて選ぶ。
...
**[4] 自動テスト**
- 成果物: `npm test` がグリーン
- 完了条件: 純粋関数の単体テスト1つ以上 ＋ API 正常系 ＋ **API 異常系（バリデーション 400 等）** ＋ 認証境界の異常系（401）
...
- **Cloudflare** — デプロイ先 [5] は**全員必達**なので事前に作っておく（アカウント作成は無料・電話番号不要）
  - 環境設定: `npx wrangler login` でブラウザ認証 → `npx wrangler whoami` で**想定アカウントか確認**
    （wrangler はプロジェクトの依存に含まれるので別途インストール不要。複数アカウント持ちは取り違えに注意）
...
> 後述の `.dev.vars` に入れて参照させる。AI が書いたコードも**読まずに承認しない**。
...
> 順番・粒度は各自で変えてよい。タイムテーブルや進行台本としても使える。
...
3. API のテスト（正常系 ＋ 異常系〔バリデーション 400 等〕＋ 認証境界の異常系〔401〕）
4. `npm test` でグリーンを確認
...
- 最低限カバーできている：
  ...
  - API の**正常系**（例: 問い合わせが 200）
  - API の**異常系**（例: 必須項目欠けで 400 などのバリデーションエラー）
  - **認証境界の異常系**（例: 未認証で 401）
- → これが緑になったら [5] デプロイへ
...
> Day 3 の**最後の 30 分**を発表会にあてる。1人3分で手短に。人数が多い場合は時間を見て調整。
