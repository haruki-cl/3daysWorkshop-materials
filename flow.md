...
| 0:35–1:00 | [1] Claude Design（LP のデザインシステム（配色・トーン・構成）を確定する。3案 → 1つ選ぶ → zip 書き出し。**マイページ等の機能ページは [2] で Claude Code が実装**） |
...
- **[1] デザイン（Day 1）**
  - ゴール: **LP のデザインシステム**（配色・トーン・コンポーネントの雰囲気）を確定する。機能ページ（マイページ・問い合わせ・サクセス）は [2] で Claude Code が実装する
  - 完了条件: LP の配色・トーン・セクション構成が決まり、**「購入する」ボタンが料金セクションへの導線**になっている。デザインを zip で書き出せる
- **[2] フロント（Day 1）**
...
- 成果物: **LP のデザインシステム**（配色・トーン・コンポーネントの雰囲気）が決まった Claude Design Project。zip でエクスポートして [2] に渡せる
- 完了条件: LP の配色・トーン・セクション構成が決まり、**「購入する」ボタンが料金セクションへの導線**になっている。デザインを zip で書き出せる
- 落とし穴: ① ピクセル完璧を狙わない ② **マイページ・問い合わせ・サクセスは指定しない**（Claude Design はLPのみ。他ページは [2] で Claude Code が実装する）③ 指定するのは「中身（機能・売り）」と「LPに含めるセクション」
...
完成後のマージ:
```
dev ブランチを main にマージして、main をリモートに push して。
```
...
## 画面構成とナビゲーション定義（[1] 着手前に確認する）

> **[1] Claude Design を始める前に、この定義を固める。** あいまいなまま渡すと「購入する」ボタンが問い合わせに飛んだり、マイページがダッシュボード型（問い合わせ数・AI解決率など）になったりする。

### ページ構成

| URL | ページ名 | 役割 |
|---|---|---|
| `/` | LP | ヒーロー・特徴・料金・FAQ。CTAから購入フローへ |
| `/contact` | 問い合わせ | フォーム送信（`POST /api/contact`） |
| `/mypage` | マイページ | アカウント情報・ライセンスキー・サブスクリプション |
| `/success` | 購入完了 | 購入完了メッセージ（Day 3 [7] 決済連携後に活用） |

### ナビゲーション・ボタンの遷移先

| ボタン / リンク | 設置場所 | 遷移先 |
|---|---|---|
| 特徴 | グローバルナビ | `/`（特徴セクションへスクロール） |
| 料金 | グローバルナビ | `/`（料金セクションへスクロール） |
| FAQ | グローバルナビ | `/`（FAQセクションへスクロール） |
| お問い合わせ | グローバルナビ | `/contact` |
| マイページ | グローバルナビ | `/mypage` |
| **購入する** | グローバルナビ・ヒーロー CTA | `/`（料金セクション）→ Day 3 [7] 後は Stripe Checkout |

> ⚠️ **CTA ボタンのテキストは「購入する」**。「無料で試す」は使わない。クリック先はコンタクトページではなく**料金セクション（または購入画面）**。

### マイページに表示する情報

マイページは「ダッシュボード（問い合わせ数・解決率などの運用指標）」ではなく、**購入者向けのアカウント管理画面**として作る。以下のプロンプトを [2] フロント実装時にそのまま Claude Code に渡す:

```
マイページ（/mypage）を実装して。表示する内容は以下の3ブロック。
...
【ライセンスキー】
- マスク表示（例: GMC-●●●●-●●●●-XXXX）
- 「メールで送信」ボタン（クリックで登録メールに送る想定）
- 「セキュリティのため全体は表示していません」という説明テキスト

【サブスクリプション】
- 現在のプラン（例: 年間プラン）
- 次回更新日（例: YYYY/MM/DD）
- 「プランを変更」「支払い方法を変更」「解約する」ボタン

この段階では API がないので、すべてモックデータで表示して。
```

> Day 2 はモック認証でこの表示を固定値で出す。Day 3 [7] Stripe・[8] Clerk で実データに切り替える。

---

## [1] Claude Design でサイトデザイン作成

**ゴール**: LP のデザインシステム（配色・トーン・コンポーネントの雰囲気）を確定し、zip でエクスポートする。**作るのは LP（`/`）のみ**
...
> **なぜ LP だけなのか**
> Claude Design はデザインの方向性（配色・フォント・余白感）を決めるのに優れている。一方、マイページ・問い合わせ・サクセスは「何を表示するか」が機能的に決まっているページ。前のセクションで定義した画面仕様を Claude Code に渡して実装させる方が、意図通りの画面になりやすい。

### 流れ

1. Claude Design に「**LP だけ**」を生成させる
   - 伝えること: 「何のサービスか・誰向けか・何を売るか・LP に含めるセクション」
   - 例（玄米茶）:
     ```
     日本茶サブスク SaaS「玄米茶」のランディングページを作って。
     LP に含めるセクション: ヒーロー（キャッチコピー + CTA）・特徴・料金プラン（月額 / 年額）・FAQ・フッター。
     CTA ボタンのテキストは「購入する」とし、クリックしたら料金セクションにスクロールする導線にして。
     ```
   - **「マイページ」「問い合わせ」「サクセス」は指定しない**（ここでは LP のみ）
   - Claude Design がトーン・配色について質問してきたら答える。**自分から細かく指定しなくてよい**
   - **1回のプロンプトでデザイン案を3つ**出してくれる

2. **出てきた3案を見比べて1つ選ぶ**（最初の1案でなんとなく進めず、**3案を見比べてから決める**）
...
4. 仕上がった LP デザインを **Claude Design の Project を zip でエクスポート**しておく
   - 操作: 画面右上の「Share」ボタン → 「Project archive (.zip)」を選んでダウンロードする
   - → これを [2] でそのまま Claude Code に渡してデザインシステムを引き継がせる。**色やフォントを別途テキストで言語化する必要はない**（デザインそのものを渡す方が早くて正確）

   ![Claude Design のエクスポート画面（Project archive .zip を選択）](img/ClaudeDesignShareZip.png)

### ポイント

- **「LP の雰囲気が決まった」で十分**。完璧なカンプは目的ではない
- 指定するのは **「中身（機能・売り）」「LP に含めるセクション」「CTA ボタンのテキストと遷移先」**
- **「購入する」ボタンの行き先**（料金セクションへのスクロール）だけは明示する
- **他ページは指定しない**。Claude Design はデザインシステム確定だけに使う

### ✅ 完了の定義（これが満たせたら [2] へ）

- LP の配色・トーン・セクション構成が決まっている
- **「購入する」ボタンが料金セクションへの導線**になっている（コンタクトに飛ばない）
...
1. フロント専用ディレクトリにプロジェクト雛形を Claude Code に作らせる
   - フロント専用のディレクトリで `claude` コマンドで Claude Code を起動する
   - Claude Code へのプロンプト例:
     ```
     Vite + React + TypeScript + Tailwind CSS v4 + shadcn/ui で SPA を作って。
     ページは `/`（LP）`/contact`（問い合わせ）`/mypage`（マイページ）`/success`（購入完了）の4つ。
     React Router でルーティングして。
     ```

2. [1] でエクスポートした **Claude Design の Project（zip）を展開して渡す**
   - zip を手動で展開してから Claude Code に渡す（Claude Code は zip を自動展開しない）
   - 渡し方の例: 「このデザインで LP を実装して」と展開先フォルダのパスを添えて依頼する
   - 言語化テキストではなく**デザインそのもの**を渡す。色・フォント・余白の指定はそこから AI が拾う

3. **LP だけでなく全画面（`/contact` `/mypage` `/success`）まで実装させる**
   - **マイページは「画面構成とナビゲーション定義」に記載した内容**（アカウント情報・ライセンスキー・サブスクリプション）で作る。Claude Code へのプロンプト例:
     ```
     マイページ（/mypage）を実装して。表示する内容は以下の3ブロック。

     【アカウント】メールアドレス・契約ステータス（「✓ 契約中」バッジ）
     【ライセンスキー】マスク表示（例: GMC-●●●●-●●●●-XXXX）＋「メールで送信」ボタン
     【サブスクリプション】現在のプラン・次回更新日・「プランを変更」「支払い方法を変更」「解約する」ボタン

     この段階では API がないので、モックデータで表示して。
     ```
   - 他ページも同様に、[1]のデザインシステム（配色・フォント）を引き継ぎながらモックデータで実装させる
   - **お問い合わせ(`/contact`)**: フォーム（名前・メール・本文）＋送信ボタン
   - **購入完了(`/success`)**: 購入完了メッセージ＋マイページへのリンク

4. 動作確認は Claude Code に依頼する: 「`npm run dev` を実行して動作確認して」または自分で起動して HMR で「AI に直させる → 即見る」を会話で繰り返す
...
- 完了したら Claude Code に依頼する:
  ```
  [2] フロント実装の作業をコミットして dev ブランチに push して。
  ```
...
- push 前に `.gitignore` が存在し、`node_modules` と `dist` が除外されているか Claude Code に確認させる
  - 例:「`.gitignore` に `node_modules` と `dist` が入っているか確認して」
- **dev ブランチに push できているか全員で確認**。Claude Code へのプロンプト例:
  ```
  今日の作業を "day1 front" というメッセージでコミットして、dev ブランチに push して。
  ```
  - リポジトリをまだ作っていない人は先に ① GitHub で空リポジトリを作成 → ② Claude Code に「リポジトリを初期化して main ブランチに push して」と依頼 → ③ 「dev ブランチを切って push して」と依頼する（詳細は [Git の進め方](#git-の進め方dev--main) 参照）
  - （※完成品・雛形は配布しない方針なので、各自の保全が頼り）
...
1. **Hono + Cloudflare Workers** のプロジェクトを Claude Code に作らせる（`genmaicha_back/`）
   - Claude Code へのプロンプト例:
     ```
     Hono + Cloudflare Workers + TypeScript でバックエンドを作って。
     データ保存に Cloudflare D1（SQLite）を使う。
     `.gitignore` も作って（node_modules / dist / .dev.vars を除外する）。
     ```
...
3. **API をまとめて実装させる**
   - 必要な機能を**日本語でまとめて伝え、Claude にいっぺんに実装させてよい**。API の形（パス・入出力）の設計も AI に任せてよい
   - Claude Code へのプロンプト例:
     ```
     Hono で以下の API を実装して（バックエンドは Hono + Cloudflare Workers + D1）。

     1. POST /api/contact
        - 名前・メール・本文を受け取って D1 に保存する
        - 必須項目が空なら 400 エラーを返す

     2. GET /api/me（モック認証）
        - リクエストヘッダ X-User-Email のメールアドレスを信頼して、
          そのユーザーの情報（メール・プラン・ライセンスキー）を返す
        - ヘッダがなければ 401 を返す
        - あとで Clerk 認証に差し替えやすい作りにして

     3. POST /api/verify-license
        - ライセンスキーを受け取って有効かどうかを返す

     テーブル定義（SQL）は migrations/ ディレクトリに作って。
     ```
   - **保存するデータの中身だけ先に決めておく**と AI が迷わない（例: 問い合わせ＝名前／メール／本文、ライセンス＝キー／メール／プラン／発行日時）
...
- 完了したら Claude Code に依頼する:
  ```
  [3] バック実装の作業をコミットして dev ブランチに push して。
  ```
...
- 完了したら Claude Code に依頼する:
  ```
  [4] 自動テスト実装の作業をコミットして dev ブランチに push して。
  ```
...
- 完了したら Claude Code に依頼する:
  ```
  [5] デプロイ完了の作業をコミットして dev ブランチに push して。必要なら dev を main にマージして。
  ```

### Day 2 クロージング

- 全員が「テスト グリーン + 公開 URL」に到達しているか確認
- **dev に push 済みかを確認**。Claude Code へのプロンプト例:「今日の作業をコミットして dev ブランチに push して」（必要なら「dev → main にマージして安定版を残して」と依頼する）
...
- 完了したら Claude Code に依頼する:
  ```
  [6] メール機能実装の作業をコミットして dev ブランチに push して。
  ```
...
- 完了したら Claude Code に依頼する:
  ```
  [7] 決済機能実装の作業をコミットして dev ブランチに push して。
  ```
...
- 完了したら Claude Code に依頼する:
  ```
  [8] Clerk 認証実装の作業をコミットして dev ブランチに push して。
  ```
...
- 完了したら Claude Code に依頼する:
  ```
  [9] 運用保守実装の作業をコミットして dev ブランチに push して。
  ```
...
- 完了したら Claude Code に依頼する:
  ```
  [10] セキュリティ修正の作業をコミットして dev ブランチに push して。必要なら dev を main にマージして。
  ```
